...
const userRole = localStorage.getItem("userRole");
  ...
  const getStatusCount = async () => {
    try {
      const response = await axios.get('http://localhost:5100/api/getStatusCount', {
        headers: {
  ...
  const getStatusCountByUser = async () => {
    try {
      const response = await axios.get('http://localhost:5100/api/getStatusCountByUser', {
        headers: {
          'Authorization': `Bearer ${token}`  
        }
      });
  ...
  const getUsersCount = async () => {
    try {
      const response = await axios.get('http://localhost:5100/api/getUsersCount', {
        headers: {
          'Authorization': `Bearer ${token}`  
        }
      });
  ...
  const getTrainerCount = async () => {
    try {
      const response = await axios.get('http://localhost:5100/api/getTrainerCount', {
        headers: {
          'Authorization': `Bearer ${token}`  
        }
      });
  ...
  const getMembershipCount = async () => {
    try {
      const response = await axios.get('http://localhost:5100/api/getMembershipCount', {
        headers: {
          'Authorization': `Bearer ${token}`  
        }
      });
          ...
          <Card className="welcome-card">
            <Card.Body className="d-flex align-items-center">
              <div className="welcome-image">
                <img src={gymAdmin} alt="Admin" className="img-fluid rounded-circle" />
              </div>
              <div className="welcome-text ms-4">
  ...
  <Card className="mb-4 shadow rounded-4 border-0">
    <Card.Body className="text-center d-flex flex-column align-items-center justify-content-center">
      ...
      <div className="p-2 rounded-3 bg-light" style={{ width: '100%', maxWidth: '350px' }}>
...
<Col md={4}>
  <Card className="mb-4 shadow-sm">
    <Card.Body>
      <Card.Title className="d-flex align-items-center">
        <FaLightbulb className="me-2 text-warning" /> Tips for a Great Day!
      </Card.Title>
      <p className="text-muted">
